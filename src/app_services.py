...
# Thread processing locks to prevent concurrent processing of the same thread
_thread_locks: Dict[str, asyncio.Lock] = {}
_locks_lock = asyncio.Lock()

async def _get_thread_lock(user_id: str, thread_name: str) -> asyncio.Lock:
    """Get or create a lock for a specific user-thread combination."""
    lock_key = f"{user_id}:{thread_name}"
    
    async with _locks_lock:
        if lock_key not in _thread_locks:
            _thread_locks[lock_key] = asyncio.Lock()
...
async def process_thread_and_create_draft(request: api_models.APISendMessageRequest) -> Optional[str]:
    ...
    """
    # Get thread-specific lock to prevent concurrent processing
    thread_lock = await _get_thread_lock(request.user_id, request.thread_name)
...
async def create_revised_draft_from_feedback(request: api_models.APIProcessFeedbackRequest) -> Optional[str]:
    ...
    # Get thread-specific lock to prevent concurrent processing
    thread_lock = await _get_thread_lock(request.user_id, thread_name)
